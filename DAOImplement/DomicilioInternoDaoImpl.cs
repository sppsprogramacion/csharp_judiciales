    ...
    public class DomicilioInternoDaoImpl : IDomicilioInternoDao
    {
        ...
        public async Task<(bool, string error)> AnularDomicilio(int idDomicilio, string dataAnular)
        {
            ...
            try
            {
                ...
                // Enviar la solicitud HTTP POST
                HttpResponseMessage httpResponse = await this.httpClient.PutAsync(url_base + "/domicilios-interno/anular?id_domicilio=" + idDomicilio, content);
                ...
                if (httpResponse.IsSuccessStatusCode)
                {
                    ...
                    {
                        return (false, "No se pudo anular el domicilio");
                    }
                }
                else
                {
                    string errorMessage = await httpResponse.Content.ReadAsStringAsync();
                    var mensaje = JObject.Parse(errorMessage)["message"]?.ToString();
                    return (false, $"Error al anular el domicilio: {mensaje}");
                }
            }
            catch (HttpRequestException httpRequestException)
            {
