...
namespace DAOImplement
{
    public class HistorialProcesalDaoImplement : IHistorialProcesalDao
    {
        ...
        public async Task<(bool, string error)> EditarHistorial(int idHistorial, string historial)
        {
            string token = SessionManager.Token; // Aquí pones tu token real
            ...
            try
            {
                ...
                // Crear el contenido de la solicitud HTTP
                StringContent content = new StringContent(historial, Encoding.UTF8, "application/json");

                // Enviar la solicitud HTTP POST
                HttpResponseMessage httpResponse = await this.httpClient.PutAsync(url_base + "/historial-procesal/" + idHistorial, content);
                ...
                if (httpResponse.IsSuccessStatusCode)
                {
                    ...
                    {
                        return (false, "No se pudo modificar el historial");
                    }
                }
                else
                {
                    string errorMessage = await httpResponse.Content.ReadAsStringAsync();
                    var mensaje = JObject.Parse(errorMessage)["message"]?.ToString();
                    return (false, $"Error al modificar el historial: {mensaje}");
                }
            }
            catch (HttpRequestException httpRequestException)
            {
